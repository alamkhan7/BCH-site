	...
	<div class="row hc_blog_wrapper">		
		...
		<?php 	$paged = (get_query_var('paged')) ? get_query_var('paged') : 1;
			...
			<div class="hc_blog_section" id="post-<?php the_ID(); ?>" <?php post_class(); ?>>	
				...
				<div class="hc_blog_post_content"><p><?php the_content( __( 'Read More' , 'medical-treatment' ) ); ?></p></div>
				<?php  wp_link_pages();  ?>				
			</div>	
			<?php endwhile ?>
			<div class="hc_blog_pagination">
				<div class="hc_blog_pagi">
			  <?php previous_posts_link( __('<span class="meta-nav">&larr;</span> Older posts', 'medical-treatment') ); ?>
			  <?php next_posts_link( __( 'Newer posts <span class="meta-nav">&rarr;</span>', 'medical-treatment' ), $post_type_data->max_num_pages ); ?>
			   </div>
			</div>
		</div>
	</div>		
